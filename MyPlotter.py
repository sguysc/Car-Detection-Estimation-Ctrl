...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 15 11:43:18 2021
...
from stonesoup.models.base import LinearModel, NonLinearModel
...
class MyPlotter:
    ...
    def plot_measurements(self, measurements, mapping, measurement_model=None, **kwargs):
        ...
        # Generate legend
        self.ax.legend(handles=self.handles_list, labels=self.labels_list)
        # print(f'detected {i} measurements alltogether')
        ...
        if uncertainty:
            ...
            for track in tracks:
                ...
                for state in track:
                    ...
                    # GUY
                    u, w, v = np.linalg.svd(HH @ state.covar @ HH.T)
                    ...
                    # except:
                        #it sometimes fails because it gets imaginary numbers
                        # due to rounding errors. it occurs when the ellipse is actually
                        # a triangle with similar eigenvalues
